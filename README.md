...
**Data & Scraping**
* `stack_exchange_latex_scrape.py` - Scraper used to extract raw Q&A data from the Stack Exchange API. <!-- -->
* `stackexchange_enhanced_dataset.csv` - The final processed dataset utilized for all models and analyses. <!-- -->

**Python Analysis (Machine Learning)**
* `requirements.txt` - Python dependencies list. <!-- -->
* `data_exploration.ipynb` - Jupyter notebook for initial exploratory data analysis (EDA). <!-- -->
* `ml_models.py` - Core machine learning pipeline handling preprocessing, training, and evaluation for the Random Forest and XGBoost predictive models. <!-- -->
* `question5_bootstrap.py` - Script executing the paired post-hoc bootstrap analysis to statistically compare ML and linear regression performance. <!-- -->

**R Analysis**
* `BEVOCI.R` - Handles the theoretical BEVoCI linear regressions, assumption diagnostics (Shapiro-Wilk, Breusch-Pagan, HC3), and plot generation. <!-- -->

**Outputs & Checkpoints**
* `plots/` - Directory containing the generated regression visualizations. <!-- -->
* `saved_models_base/` - Cached checkpoint files for the baseline ML models. <!-- -->
* `saved_models_metacognitive/` - Cached checkpoint files for the metacognitive ML models. <!-- -->
* `question5_bootstrap_results.csv` - The exported results table containing confidence intervals and empirical p-values from the bootstrap script. <!-- -->

## Authors

Maya Rozenshtein, Elad Yosef, and Thomas Erlich  
